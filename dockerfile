...
# RUN apt-get update && apt-get install -y gnupg apt-transport-https wget software-properties-common gpg lsb-release
RUN wget -q https://qgis.org/downloads/qgis-2021.gpg.key -O- | gpg --no-default-keyring --keyring gnupg-ring:/etc/apt/trusted.gpg.d/qgis-archive.gpg --import && \
